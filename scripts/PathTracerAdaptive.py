...
_cfg = config.PRESETS[config.CURRENT_PRESET]["proposed"]
...
try:
    if ENABLE_AUTO_CAPTURE:
        output_dir = OUTPUT_BASE + "/PSNR/proposed_" + CAMERA_ANIMATION
        os.makedirs(output_dir, exist_ok=True)   # 캡처 폴더가 없으면 생성
        m.frameCapture.outputDir    = output_dir
        m.frameCapture.baseFilename = "proposed"
...
try:
    if ENABLE_FRAMETIME_MEASUREMENT:
        ...
        output_dir = OUTPUT_BASE + "/frame_time/proposed_" + CAMERA_ANIMATION
        os.makedirs(output_dir, exist_ok=True)   # 캡처 폴더가 없으면 생성
        m.frameCapture.outputDir    = output_dir
        m.frameCapture.baseFilename = "proposed"
